...
void display()  
{
    ...
    //glRotatef(cam_angle, 0.0, 1.0, 0.0);		//rotate the whole scene
...
void initialize()
{
	...
	glLoadIdentity();
	gluPerspective(70, 1, 50, 5000);  //The camera view volume  
}

void special(int key, int x, int y)
{
    if(key == GLUT_KEY_LEFT) cam_angle++;
    else if(key == GLUT_KEY_RIGHT) cam_angle--;
	else if(key == GLUT_KEY_UP) {
        cam_z -= (10*cos(cam_angle * cdr));
        cam_x -= (10*sin(cam_angle * cdr));
    }
